...
显示器选择新增“自动（主显示器）”，并将它设为未配置用户的默认值；原有
“自动（跟随鼠标）”保留为独立选项，也仍可固定到某一块显示器。

- “主显示器”严格按 macOS 系统设置中带菜单栏的显示器判断，不再受当前键盘焦点
  窗口影响；设置页的星标与实际路由使用同一判据。
- 主显示器被全屏内容遮挡时仍会回退到鼠标所在屏，避免通知落在用户看不到的位置。
- 已经手动保存过显示器偏好的用户保持原设置，不被版本升级覆盖。

## [1.2.4] — 2026-08-20

### 🚨 仅记录（全部放行）

风险引擎之外新增一个明确标红的危险旁路：开启后，普通审批请求不再等待 AI 判断、
Beacon 或飞书，直接允许执行，同时把脱敏后的请求与 `audit-only` 决定完整记入审计。

- 开启必须输入确认短语，可选“本次会话”“1 小时”或“永久”；1 小时档按绝对时间
  到期，电脑休眠后也不会延长。
- 设置导航、标题栏和 Beacon 持续显示红色状态，避免忘记仍处于全部放行模式。
- Memora 自身数据、审计文件、审批配置、Hook 与安装包保留最后一道硬性人工确认，
  不会再被 hard-allow 或全托管二次放行。
- 自身保护判定覆盖 Edit、Write、NotebookEdit、Codex 多文件 patch、shell 重定向、
  相对路径以及常见 cp / rg / Git / SQLite 变体。

这是为明确知道风险、只希望保留审计的人准备的模式，不是更安全的自动审批档位。

### 🧠 Continue 上下文去重

项目同时存在 `CLAUDE.md` 与指向它的 `AGENTS.md` 软链时，Continue 不再把同一份项目
指令重复写进上下文。去重依据是真实文件路径，不会误删内容相同但彼此独立的文件。

### 📦 发布包

本机安装副本会在公证成功后补上票据，与 DMG 中分发给用户的 App 保持一致；离线首次
启动不再依赖向 Apple 在线查询公证状态。公证票据失败时也会保留可诊断的具体原因。

## [1.2.3] — 2026-08-18

> 上一版审了两条主链路，这一版把剩下的十个模块逐个过了一遍 —— 数据层、界面、
> 通知通道、Beacon、hook、桥接层、运维脚本、OpenCode 插件。每个模块都是
> 「复审 → 修 → 再复审」循环到无 P2 以上问题才进下一个。
>
> 修的几乎全是**不报错的错**：时间戳静默变成 1970、会话时长少算 5 天、
> monorepo 子目录的文件历史永远是空的、Workspace 说创建成功但目录里没有代码。
> 这类问题界面上一切正常、日志里一片干净，只有把数据横向对一下才看得见 ——
> 所以这一版也留下了两个体检脚本，以后能自己查。

### 🕐 时间戳与会话时长

- **Cursor 的消息时间戳全是 1970-01-01**（本机 23 条）。原因是取值失败时的兜底
  写法在这条路上永远不生效 —— 缺字段取到 0，而 0 在时间函数里是合法的
  「1970 那一刻」，不是「没有值」。时间轴上它们排在所有内容最前面，
  Continue 组装上下文时顺序也是错的。现在回退到会话开始时间。
- **会话结束时间会被旧记录覆盖**（本机 2 个会话，最严重的少算 133 小时）。
  Claude Code 用 resume 续接时，同一个会话由多份记录拼成，而处理顺序不固定 ——
  旧的那份后处理就把新的结束时间盖回去了。会话时长因此显示得比实际短。
- 两类错误都会**就地订正**：这一版启动时会把已经入库的错值修回来，不删任何正文。

### 📁 monorepo 子目录

`MerchantAI/cli`、`server`、`chat` 这类没有自己 `.git` 的子项目（本机 40 个项目
里有 8 个），此前：

- **文件历史面板永远是空的** —— 判断「在不在 git 仓库里」时只看目录下有没有
  `.git`，子目录一进门就返回空
- **Continue 与 Workspace 的提交顺序是错的** —— git 的提交时间带作者本地时区，
  按字符串排序不等于按时间排序。本机 6865 条提交里有 3344 条位次不同，
  在「只取最近 20/50 条」的边界上会漏掉真正较新的提交

### ⚠️ Workspace 不再假装成功（行为变更）

选一个**不是 git 仓库根**的目录创建 Workspace 时，此前会建一个空目录、写好上下文
文件、然后返回「创建成功」—— 而你进去发现没有代码。

现在会明确报错，提示选择仓库根。monorepo 子目录与真正没有 git 的目录都是如此。

### 🔗 提交与会话的关联

- 关联写入前会确认那个会话真的在库里。此前不检查，于是会留下指向不存在会话的
  关联（本机 1 条）—— 界面上显示「关联到某会话」，点进去是空的。**留空会如实
  归进「其他提交」，假关联却让人以为查得到。**
- 已有的这类关联会在启动时清掉，只清关联不删提交记录。

### 🧹 其余九个模块

界面层收口了异步结果的时序（切换项目/页面时，在途的旧结果不再覆盖新的）；
通知通道以回执落盘为准判定「谁先响应」；Beacon 的请求队列与原生窗口状态对齐；
hook 收紧了审批协议与提交水位；桥接层不再用空列表/false 伪装失败；运维脚本
「检查不完整」不再报成 clean；OpenCode 改为直接以它自己的数据库为准，
并补齐了四类 token 的统计。

### 🩺 两个体检脚本

`scripts/ingest-health-check.sh` 与 `scripts/core-health-check.sh`。都只读，
跑在数据库的一致性快照上，不跟正在运行的 App 抢锁。

这两个脚本第一次跑就查出了上面那几条 —— 采集链路历史上出过四次事故
（消息重复、索引膨胀撑爆磁盘、时间戳放大 1000 倍），每一次都是等发现异常了
才回头查。现在能主动查了。
